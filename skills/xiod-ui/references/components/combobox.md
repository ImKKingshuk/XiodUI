...
- `defaultInputValue` — The uncontrolled input value when initially rendered.
...
- `defaultValue` — The uncontrolled selected value of the combobox when it's initially rendered.
...
- `inline` — Whether the list is rendered inline without using the component's own popup.
  Specify `open` unconditionally in conjunction with this prop so the list is considered visible: `<Combobox inline open>`
  In a `Combobox` > `Dialog` composition, bind the Combobox's `open` and `onOpenChange` props to the `Dialog`'s `open` and `onOpenChange` state instead so the component resets its transient state (filter query, highlighted item, and input value) when the dialog closes.
