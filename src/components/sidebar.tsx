...
import { ViewSidebarLeft as PanelLeftIcon } from "xiod-icons/icons/ViewSidebarLeft";
...
function Sidebar({
  ...
  const isSheet = isMobile || variant === "modal";
        ...
        <SheetPrimitive.Portal>
          ...
          >
            <SheetPrimitive.Popup
              className={cn(
                "relative flex max-h-full min-h-0 w-(--sidebar-width) min-w-0 flex-col bg-sidebar text-sidebar-foreground shadow-lg/5 transition-[opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:shadow-[0_1px_--theme(--color-black/4%)] data-ending-style:opacity-0 data-starting-style:opacity-0 max-sm:before:hidden dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
                side === "left"
                  ? "data-ending-style:-translate-x-8 data-starting-style:-translate-x-8 border-e"
                  : "data-ending-style:translate-x-8 data-starting-style:translate-x-8 border-s",
...
function SidebarRail({
  ...
  return (
    <button
      ...
      className={cn(
        "-translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-20 hidden w-4 transition-[color,background-color] ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=right]:left-0 sm:flex",
...
function SidebarMenuSub({
  ...
  return (
    <ul
      className={cn(
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-sidebar-border border-l ps-[calc(--spacing(2.5)-1px)] pe-2.5 py-0.5",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
...
function SidebarMenuSubButton({
  ...
  const defaultProps = {
    className: cn(
      "-translate-x-px flex h-7 min-w-0 items-center gap-2 overflow-hidden rounded-lg px-2 text-sidebar-foreground outline-hidden ring-sidebar-ring transition-[color,background-color,box-shadow] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-64 aria-disabled:pointer-events-none aria-disabled:opacity-64 [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
